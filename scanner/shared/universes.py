...
CASH_MARKET = sorted(set(NIFTY_50 + BANK_NIFTY + NIFTY_NEXT_50 + FNO_STOCKS))
...
def get_universe(name: str) -> list:
    ...
    if low in ("nse all (live ~2,200)", "nse all", "nse all (live)"):
        try:
            ...
            logger.info(
                "NSE live fetch failed, falling back to static list", exc_info=True
            )
    ...
    if low in ("bse all (live ~4,500)", "bse all", "bse all (live)"):
        try:
            ...
            logger.info(
                "BSE live fetch failed, falling back to static list", exc_info=True
            )
        return UNIVERSES.get("BSE ALL (Live ~4,500)") or NIFTY_BROAD

    if low in (
        "full market (nse+bse ~5,900)",
        "full market",
        "full market (live)",
        "all market",
    ):
        try:
            ...
            logger.info(
                "Full market live fetch failed, falling back to static list",
                exc_info=True,
            )
    ...
    if low in ("fno stocks", "fno"):
        try:
            ...
            logger.info(
                "F&O live fetch failed, falling back to static list", exc_info=True
            )
