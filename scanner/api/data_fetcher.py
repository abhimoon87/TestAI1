...
def resample_ohlcv(df: pd.DataFrame, timeframe: str) -> pd.DataFrame | None:
    """
...
def _fetch_fallback_batch(
    ...
    deadline = time.monotonic() + FALLBACK_OVERALL_TIMEOUT
    t0 = time.monotonic()
...
def fetch_batch_yfinance_stream(
    ...
    """
    Streaming generator that yields one dict per downloaded chunk.

    Each yielded dict is {ticker: DataFrame} for ~200 tickers (one yfinance
    chunk). Chunks download in parallel groups of up to MAX_PARALLEL_CHUNKS
    but are yielded individually as each finishes, so the caller paints the
    first rows after the fastest chunk (~10-20s) instead of waiting for the
        ...
        for batch_start in range(0, len(chunks), MAX_PARALLEL_CHUNKS):
            ...
            while pending:
                ...
                for future in completed:
                    ...
                    if not chunk_res:
                        continue
                    # Stream each chunk the moment it lands instead of waiting
                    # for the whole group: first paint after the fastest chunk
                    # rather than the slowest one. Already-yielded chunks are
                    # unaffected by a later cancel (same data as before, earlier).
                    batch_results.update(chunk_res)
                    cumulative += len(chunk_res)
