...
def _weekly_cache_key(df: pd.DataFrame) -> int:
    """Content-based cache key — survives copies and attrs mutation."""
    try:
...
def to_weekly(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    data (e.g. Phase-1 and Phase-2 scoring) skip the expensive
    copy-resample.  The cache key is based on the close column content, so
    copies of the same DataFrame share the cache.  The cache is bounded;
    when full it is cleared and recomputed on demand.
    """
...
def check_hp_volume(volume, xo: dict, settings: dict | None) -> bool:
    ...
    bars_ago = xo["bars_ago"]
    if bars_ago >= len(volume):
        return True
    vol_idx = -bars_ago if bars_ago > 0 else -1
...
def _last_values(ind: dict) -> dict:
    """
