...
def _generate_trade_chart(
    t: TradeResult, stock_data: dict, chart_w: int = 320, chart_h: int = 120
) -> str:
    """
    ...
    if t.stop_loss > min_p:
        sly = sy(t.stop_loss)
        svg_parts.append(
            f'<line x1="{margin_l}" y1="{sly}" x2="{chart_w - margin_r}" y2="{sly}" stroke="#ef4444" stroke-width="0.7" stroke-dasharray="3,3" opacity="0.6"/>'
        )
        svg_parts.append(
            f'<text x="{chart_w - margin_r - 2}" y="{sly + 10}" fill="#ef4444" font-size="7" text-anchor="end" opacity="0.8">SL</text>'
        )
...
def save_trades_csv(
    trades: list[TradeResult], filepath: str = "Reports/backtest_trades.csv"
):
