...
class SettingsViewMixin:
    ...
    # (key, label, kind, min, max) — kind in {"int", "float", "ma_type"}.
    ...
    _SETTINGS_SPEC: ClassVar[list] = [
        (
            ...
            [
                (
                    "reduce_motion",
                    "Reduce motion (no transitions/animations)",
                    "bool",
                    None,
                    None,
                ),
        ...
        (
            ...
            [
                ...
                (
                    "volume_participation_len",
                    "Volume participation window (bars)",
                    "int",
                    1,
                    50,
                ),
        ...
        (
            ...
            [
                ...
                (
                    "sideways_strong_move_pct",
                    "Strong-move sideways guard (% 1M move)",
                    "float",
                    0,
                    50,
                ),
    ...
    def _settings_input(self, key, label, kind, lo, hi):
        ...
        if kind == "bool":
            ...
            ctrl = themed_dropdown(opts, cur if cur in opts else opts[0], c, width=160)
        elif kind == "entry_mode":
            opts = ["classic", "high_probability", "custom"]
            cur = str(self.settings.get(key, "classic"))
            ctrl = themed_dropdown(
                opts, cur if cur in opts else "classic", c, width=190
            )
        else:
            ctrl = ft.TextField(
    ...
    def _build_settings_view(self):
        ...
        for title, fields in self._SETTINGS_SPEC:
            rows = [
                self._settings_input(key, label, kind, lo, hi)
        ...
        header = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "Indicator parameters consumed by the HMA×EMA engine — applied on Save.",
                        size=11,
                        color=c["text_dim"],
                    ),
                ],
        ...
        footer = ft.Row(
            [
                ...
                ft.Button(
                    content=ft.Text("Save Settings", size=13),
                    bgcolor=c["green"],
                    color=c["on_accent"],
        ...
        return ft.Column(
            controls=[
                ...
                body,
                ft.Container(
                    content=audit_row, padding=_padding_only(left=8, right=8, top=10)
                ),
                ft.Container(
                    content=self.stale_audit_lbl,
                    padding=_padding_only(left=8, right=8, bottom=6),
                ),
                ft.Container(
                    content=footer, padding=_padding_only(top=6, bottom=16, right=8)
                ),
            ],
            spacing=0,
            expand=True,
    ...
    def _save_settings_page(self, e=None):
        ...
        for key, ctrl in self._settings_inputs.items():
            ...
            if kind == "bool":
                updates[key] = bool(ctrl.value)
                continue
            if kind in ("ma_type", "entry_mode"):
                val = ctrl.value
                ok_opts = (
                    self._MA_TYPES
                    if kind == "ma_type"
                    else ["classic", "high_probability", "custom"]
                )
        ...
        try:
            ...
            logger.info("Failed to save settings", exc_info=True)
