...
def test_refresh_empty_hides_clear_button(monkeypatch):
    monkeypatch.setattr(cache_manager, "enrichment_size", lambda: 0)
    app = _make_app()
...
def test_refresh_before_sidebar_built_is_noop(monkeypatch):
    """Early-startup call (no widgets yet) must not raise or touch cache_manager."""

    def boom():
        raise AssertionError("cache_manager must not be touched before sidebar exists")
...
def test_clear_error_is_logged_and_ui_still_refreshes(monkeypatch):
    ...
    monkeypatch.setattr(cache_manager, "enrichment_clear", boom)
    monkeypatch.setattr(cache_manager, "enrichment_size", lambda: 5)
    app = _make_app()
...
def test_price_refresh_shows_stale_count_and_reveals_prune(monkeypatch):
    monkeypatch.setattr(cache_manager, "cache_health", lambda: _price_health(5125, 3))
    app = _make_app()
...
def test_price_refresh_clean_hides_prune(monkeypatch):
    monkeypatch.setattr(cache_manager, "cache_health", lambda: _price_health(5125, 0))
    app = _make_app()
...
def test_price_refresh_empty_state(monkeypatch):
    monkeypatch.setattr(cache_manager, "cache_health", lambda: _price_health(0, 0))
    app = _make_app()
...
def test_manual_prune_forces_sweep_logs_and_refreshes(monkeypatch):
    ...
    monkeypatch.setattr(cache_manager, "prune_stale_cache", fake_prune)
    monkeypatch.setattr(cache_manager, "cache_health", lambda: _price_health(0, 0))
    app = _make_app()
...
def test_manual_prune_error_is_logged_and_ui_still_refreshes(monkeypatch):
    ...
    monkeypatch.setattr(cache_manager, "prune_stale_cache", boom)
    monkeypatch.setattr(cache_manager, "cache_health", lambda: _price_health(10, 2))
    app = _make_app()
