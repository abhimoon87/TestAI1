...
class TestBuildSettingsView:
    def test_build_settings_view_makes_one_input_per_spec_field(self):
        ...
        assert app._settings_inputs["entry_mode"]._settings_kind == "entry_mode"
        assert "theme" not in app._settings_inputs  # dark-only UI: no theme field
    ...
    def test_cancel_closes_dialog_without_applying(self):
        ...
        assert app.page.shown == [dlg]  # no apply ran
...
class TestDataRowColoring:
    def test_score_coloring_and_threshold_highlight(self):
        ...
        # Score cell: just value text (no bar)
        above_score = _cell(above, 2)
        assert above_score.value == "72"
        assert above_score.color == c["green"]
        # 20 -> red
        below_score = _cell(below, 2)
        assert below_score.value == "20"
        assert below_score.color == c["red"]
...
class TestNewsPrefetch:
    """NewsPrefetcher attaches prefetched stories to the top rows."""
    ...
    def _app(rows):
        ...
        app._news_prefetcher = NewsPrefetcher(
            get_results=lambda: app.all_results,
            results_lock=app._results_lock,
            get_epoch=lambda: app._scan_epoch,
            get_cancelled=lambda: app._scan_cancelled,
            on_update=lambda: app._deferred.append(app._flush_news_badges),
            on_log=lambda m: app.logged.append(m),
        )
        return app
