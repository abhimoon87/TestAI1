"""Unit tests for scanner.api.data_fetcher — data fetching and resampling.
...
from scanner.api.data_fetcher import (
    CHUNK,
...
class TestBatchDownloadCache:
    ...
    def test_all_cached_skips_yfinance_entirely(self):
        ...
        with patch("scanner.api.data_fetcher._get_cached", side_effect=fake_get):
            with patch("scanner.api.data_fetcher._set_cached") as mock_set:
                with patch.dict("sys.modules", {"yfinance": mock_yf}):
                    result = fetch_batch_yfinance(["RELIANCE", "TCS"], period="1y")
    ...
    def test_fresh_download_is_cached_for_the_next_scan(self):
        ...
        with patch("scanner.api.data_fetcher._get_cached", return_value=None):
            with patch("scanner.api.data_fetcher._set_cached", side_effect=fake_set):
                with patch.dict("sys.modules", {"yfinance": mock_yf}):
                    result = fetch_batch_yfinance(["RELIANCE", "TCS"], period="1y")
...
class TestStreamYieldsPerChunk:
    """The stream generator yields each chunk as it lands (not per group).

    First paint must arrive after the fastest chunk, not the slowest of the
    group — otherwise a single rate-limited chunk holds up ~1600 tickers and
    the grid looks stuck for minutes.
    """

    def test_first_yield_beats_slowest_chunk(self):
        ...
        def staggered_download(*a, **kw):
            requested = [t[:-3] for t in a[0]]  # strip ".NS"
            if any(t.startswith("SLOW") for t in requested):
                _time.sleep(SLOW_SLEEP)
            return _make_yf_download_result(a[0], n=60, force_multi=True)
    ...
    def test_failed_chunk_does_not_block_sibling_yield(self):
        """A chunk whose download raises still lets its sibling through."""

        def flaky_download(*a, **kw):
            requested = [t[:-3] for t in a[0]]
            if any(t.startswith("BAD") for t in requested):
                raise RuntimeError("yahoo 429")
            return _make_yf_download_result(a[0], n=200, force_multi=True)
...
class TestScanStartStalePrune:
    ...
    def _write_stale_entry(d, name):
        ...
        df = _make_daily_ohlcv(60).reset_index(drop=True)
    ...
    def test_batch_fetch_prunes_stale_entries(self, tmp_path, monkeypatch):
        ...
        stale_pkl = _os.path.join(cache_dir, "stale1.parquet")
