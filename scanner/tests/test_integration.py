    ...
    def test_filter_and_score_pipeline(self, real_stock_data, real_index_data):
        ...
        for ticker, df in real_stock_data.items():
            filter_result = check_filter(
                df, fast_ma_type="HMA", fast_ma_len=40,
                slow_ma_type="EMA", slow_ma_len=50,
                crossover_lookback=20,
            )
...
class TestEndToEndConsistency:
    ...
    def test_score_sum_matches_total(self):
        """Sum of category scores should equal total_raw (within rounding).

        ``total`` may be normalized upward when fundamental data is missing,
        so the raw unnormalized sum is compared instead.
        """
        ...
        for ticker, df in data.items():
            ...
            if result is not None:
                ...
                assert abs(cat_sum - result["total_raw"]) < 0.2, \
                    f"{ticker}: category sum {cat_sum:.1f} != total_raw {result['total_raw']:.1f}"
                if result["fundamentals_available"]:
                    assert abs(result["total"] - result["total_raw"]) < 0.2
