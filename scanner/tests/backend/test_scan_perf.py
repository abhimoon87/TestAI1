...
class TestParallelScore:
    def test_preserves_input_order(self):
        ...
        assert [ordered[i][0]["ticker"] for i in range(8)] == [
...
class TestSmallCacheReadthrough:
    ...
    def test_enabled_missing_keys_get_live_defaults(self, monkeypatch, tmp_path):
        ...
        out, _ = self._score("PF14", df, enrich, lambda ticker: None, monkeypatch)
...
class TestScanStreamSmallParallel:
    def test_counts_order_and_single_enrich_per_ticker(self, monkeypatch, tmp_path):
        _isolated_cache(monkeypatch, tmp_path)
        dfs = {
            "PS01": _crossover_df(seed=61),
            "PS02": _flat_df(),
            "PS03": _crossover_df(seed=63),
            "PS04": _flat_df(),
        }
        tickers = list(dfs)
        enrich_calls = []
        ...
        monkeypatch.setattr(
            eng_mod, "fetch_fundamentals", lambda ticker: {"pe_ratio": 18.0}
        )
        ...
        result = eng.scan_stream(
            "TEST",
            dict(TestSmallCacheReadthrough.SETTINGS),
            period="1y",
            timeframe="D",
            trend_filter="All",
            index_symbol="NSEI",
